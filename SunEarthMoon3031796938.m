function [] = SunEarthMoon3031796938(years,framerate)
%% SunEarthMoon
% Final Project
% By: Bai Hao
...
v_mercury = 4.8e4; % orbital speed [m/s]
x_mercury = 57.91e9; % distance from sun [m]
...
%index: earth position:1, velocity:4; sun 7, 10; moon 13, 16; mercury 19, 22
...
T_index_mercury = find([1; x(:,22)].*[x(:,22);1]<=0);
...
D_moon = 0;
for i = 4:length(T_index_mercury)
    T_mercury_semi(i-3) = (t(T_index_mercury(i)-1)-t(T_index_mercury(i-2)-1))/24/60/60;
end
T_mercury = mean(T_mercury_semi);
D_mercury = 0;
for i = 2:(T_index_mercury(4)-1)
    D_mercury = D_mercury + sqrt((x(i,19)-x(i-1,19))^2+(x(i,20)-x(i-1,20))^2+(x(i,21)-x(i-1,21))^2);
end
...
for i = 1:length(t)/q
    subplot(2,2,1)
    plot3(x(1:i*q,1),x(1:i*q,2),x(1:i*q,3),'g',x(1:i*q,7),x(1:i*q,8),x(1:i*q,9),'r',x(1:i*q,13),x(1:i*q,14),x(1:i*q,15),'b',x(1:i*q,19),x(1:i*q,20),x(1:i*q,21),'c')
    ...
    legend('Earth','Sun','Moon', 'mercury')
    hold on
    plot3(x(i*q,1),x(i*q,2),x(i*q,3),'g-o',x(i*q,7),x(i*q,8),x(i*q,9),'r-o',x(i*q,13),x(i*q,14),x(i*q,15),'b-o',x(i*q,19),x(i*q,20),x(i*q,21), 'c-o')
    hold off
    ...
    subplot(2,2,3)
    plot(x(1:i*q,19),x(1:i*q,20),'g',x(1:i*q,7),x(1:i*q,8),'r')
    axis(1.5*[min(x(:,1)) max(x(:,1)) min(x(:,2)) max(x(:,2))])
    xlabel('Universal X Coordinate (m)')
    ylabel('Universal Y Coordinate (m)')
    title('Relative Mercury Orbit About Sun')
    hold on
    plot(x(i*q,19),x(i*q,20),'g-o',x(i*q,7),x(i*q,8),'r-o')
    text(1.45*min(x(:,19)),1.40*max(x(:,20)),sprintf('Orbital Period, T = %3.5g days',T_mercury))
    text(1.45*min(x(:,19)),1.25*max(x(:,20)),sprintf('Orbital Circumference, D = %3.5g gigameters',D_mercury*1e-9))
    text(1.45*min(x(:,19)),1.40*min(x(:,20)),sprintf('Time, t = %3.3g days',round(t(q*i)/24/60/60)))
    hold off
...
udot(20,1) = u(23,1);
udot(21,1) = u(24,1);
udot(22,1) = m_earth*G*(u(1,1)-u(19,1))/d_earth_mercury^3 + m_sun*G*(u(7,1)-u(19,1))/d_sun_mercury^3 + m_sat*G*(u(13,1)-u(19,1))/d_mercury_sat^3;
udot(23,1) = m_earth*G*(u(2,1)-u(20,1))/d_earth_mercury^3 + m_sun*G*(u(8,1)-u(20,1))/d_sun_mercury^3 + m_sat*G*(u(14,1)-u(20,1))/d_mercury_sat^3;
udot(24,1) = m_earth*G*(u(3,1)-u(21,1))/d_earth_mercury^3 + m_sun*G*(u(9,1)-u(21,1))/d_sun_mercury^3 + m_sat*G*(u(15,1)-u(21,1))/d_mercury_sat^3;
